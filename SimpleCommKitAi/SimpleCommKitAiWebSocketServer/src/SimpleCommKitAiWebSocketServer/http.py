"""SimpleCommKitAiWebSocketServer REST Server - HTTP API for WebSocket server control."""
...
try:
    ...
    print("Dependencies for SimpleCommKitAiWebSocketServer HTTP server are not installed.")
    exit(1)
...
from SimpleCommKitAiWebSocketServer import WebSocketServer, get_error_description
from SimpleCommKitAiWebSocketServer import __version__
class WsServerState:
    def __init__(self):
    ...
    def ensure_server(self) -> WebSocketServer:
        if self.server is None:
            ...
            self.server.set_callback_on_error(lambda code: print(
                f"[SimpleCommKitAiWebSocketServer-http] Error {code}: {get_error_description(code)}"))
...
async def lifespan(app: FastAPI):
    print("[SimpleCommKitAiWebSocketServer-http] Starting...")
    yield


app = FastAPI(title="SimpleCommKitAiWebSocketServer API",
              description="REST API to control WebSocket server using SimpleCommKitPyWebSocket",
...
@app.get("/")
def root():
    return {"message": "SimpleCommKitAiWebSocketServer API is running"}
...
def main():
    parser = argparse.ArgumentParser(description="SimpleCommKitAiWebSocketServer REST Server")
