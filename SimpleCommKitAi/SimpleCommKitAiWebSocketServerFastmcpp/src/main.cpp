/// SimpleCommKitAiWebSocketServerFastmcpp - C++ MCP server for WebSocket server
///
/// Exposes 8 MCP tools for WebSocket server start/stop, send, broadcast,
/// status, buffered message retrieval, max connections, and thread configuration.
/// Supports stdio, http, sse, and streamable-http transports.
///
/// Usage:
///   simplecommkitaiwebsocketserver-fastmcpp                                  # stdio (default)
///   simplecommkitaiwebsocketserver-fastmcpp --transport sse --port 8011      # SSE on port 8011
///   simplecommkitaiwebsocketserver-fastmcpp --transport streamable-http      # Streamable HTTP
///   simplecommkitaiwebsocketserver-fastmcpp --transport http --port 8011     # HTTP on port 8011

#include "ws_server_state.hpp"
#include "ws_server_tools.hpp"
...
using namespace SimpleCommKitAiWebSocketServerFastmcpp;
...
int main(int argc, char* argv[])
{
    ...
    // -----------------------------------------------------------------------
    fastmcpp::FastMCP app("SimpleCommKitAiWebSocketServerFastmcpp MCP Server", "1.0.0",
    ...
    if (transport == "http")
    {
        std::cerr << "[SimpleCommKitAiWebSocketServerFastmcpp] Starting HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiWebSocketServerFastmcpp] Starting SSE server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiWebSocketServerFastmcpp] Starting Streamable HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        // Default: stdio
        std::cerr << "[SimpleCommKitAiWebSocketServerFastmcpp] Starting STDIO MCP server..." << std::endl;
    ...
    std::cerr << "[SimpleCommKitAiWebSocketServerFastmcpp] Server stopped." << std::endl;
    return 0;
}
