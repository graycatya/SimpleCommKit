/// SimpleCommKitAiBleFastmcpp - C++ MCP server for BLE devices
///
/// Exposes 16 MCP tools for BLE (Bluetooth Low Energy) scanning,
/// connection, GATT read/write/notify, and descriptor operations.
/// Supports stdio, http, sse, and streamable-http transports.
///
/// Usage:
///   simplecommkitaible-fastmcpp                                  # stdio (default)
///   simplecommkitaible-fastmcpp --transport sse --port 8003      # SSE on port 8003
///   simplecommkitaible-fastmcpp --transport streamable-http     # Streamable HTTP
///   simplecommkitaible-fastmcpp --transport http --port 8003    # HTTP on port 8003

#include "ble_state.hpp"
#include "ble_tools.hpp"
...
int main(int argc, char* argv[])
{
    ...
    // -----------------------------------------------------------------------
    std::cerr << "[SimpleCommKitAiBleFastmcpp] Initializing BLE hardware..." << std::endl;
    if (!BleState::instance().init())
    {
        std::cerr << "[SimpleCommKitAiBleFastmcpp] WARNING: BLE hardware not available. "
    ...
    // -----------------------------------------------------------------------
    fastmcpp::FastMCP app("SimpleCommKitAiBleFastmcpp MCP Server", "1.0.0",
    ...
    if (transport == "http")
    {
        std::cerr << "[SimpleCommKitAiBleFastmcpp] Starting HTTP server on "
                  << host << ":" << port << std::endl;
        ...
        // Disconnect all BLE devices before stopping the server
        BleState::instance().cleanup();
        server.stop();
    }
    else if (transport == "sse")
    {
        std::cerr << "[SimpleCommKitAiBleFastmcpp] Starting SSE server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiBleFastmcpp] Starting Streamable HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        // Default: stdio
        std::cerr << "[SimpleCommKitAiBleFastmcpp] Starting STDIO MCP server..." << std::endl;
    ...
    std::cerr << "[SimpleCommKitAiBleFastmcpp] Server stopped." << std::endl;
    return 0;
}
