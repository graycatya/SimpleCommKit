...
// ===========================================================================
// UdpClientState singleton
// ===========================================================================

UdpClientState& UdpClientState::instance()
{
    static UdpClientState s;
    return s;
}

void UdpClientState::ensure_client()
{
    ...
    client_ = std::make_unique<SimpleCommKit::SimpleCommKitUdpClient>();
    install_callbacks();
}

bool UdpClientState::has_client()
{
    std::lock_guard<std::mutex> lock(client_mutex_);
    return client_ != nullptr;
}

SimpleCommKit::SimpleCommKitUdpClient* UdpClientState::get_client()
{
    std::lock_guard<std::mutex> lock(client_mutex_);
    return client_.get();
}

void UdpClientState::destroy_client()
{
    std::lock_guard<std::mutex> lock(client_mutex_);
    client_.reset();
...
void UdpClientState::install_callbacks()
{
    ...
    client_->setCallback_OnError(
        [](SimpleCommKit::ErrorCode code)
        {
            ...
            std::cerr << "[SimpleCommKitAiUdpClientFastmcpp] Error: "
...
std::vector<UdpMessageEntry> UdpClientState::drain_messages()
{
    std::lock_guard<std::mutex> lock(message_mutex_);
...
void UdpClientState::clear_messages()
{
    std::lock_guard<std::mutex> lock(message_mutex_);
    message_buffer_.clear();
}
