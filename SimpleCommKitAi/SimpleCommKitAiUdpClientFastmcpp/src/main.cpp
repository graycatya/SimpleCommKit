/// SimpleCommKitAiUdpClientFastmcpp - C++ MCP server for UDP client
///
/// Exposes 5 MCP tools for UDP client open/close, send, status,
/// and buffered message retrieval.
/// Supports stdio, http, sse, and streamable-http transports.
///
/// Usage:
///   simplecommkitaiudpclient-fastmcpp                                  # stdio (default)
///   simplecommkitaiudpclient-fastmcpp --transport sse --port 8008      # SSE on port 8008
///   simplecommkitaiudpclient-fastmcpp --transport streamable-http      # Streamable HTTP
///   simplecommkitaiudpclient-fastmcpp --transport http --port 8008     # HTTP on port 8008

#include "udp_state.hpp"
#include "udp_tools.hpp"
...
using namespace SimpleCommKitAiUdpClientFastmcpp;
...
int main(int argc, char* argv[])
{
    ...
    // -----------------------------------------------------------------------
    fastmcpp::FastMCP app("SimpleCommKitAiUdpClientFastmcpp MCP Server", "1.0.0",
    ...
    if (transport == "http")
    {
        std::cerr << "[SimpleCommKitAiUdpClientFastmcpp] Starting HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiUdpClientFastmcpp] Starting SSE server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiUdpClientFastmcpp] Starting Streamable HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        // Default: stdio
        std::cerr << "[SimpleCommKitAiUdpClientFastmcpp] Starting STDIO MCP server..." << std::endl;
    ...
    std::cerr << "[SimpleCommKitAiUdpClientFastmcpp] Server stopped." << std::endl;
    return 0;
}
