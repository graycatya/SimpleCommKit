/// SimpleCommKitAiUdpServerFastmcpp - C++ MCP server for UDP server
///
/// Exposes 6 MCP tools for UDP server start/stop, send, broadcast, status,
/// and buffered message retrieval.
/// Supports stdio, http, sse, and streamable-http transports.
///
/// Usage:
///   simplecommkitaiupdserver-fastmcpp                                  # stdio (default)
///   simplecommkitaiupdserver-fastmcpp --transport sse --port 8102      # SSE on port 8102
///   simplecommkitaiupdserver-fastmcpp --transport streamable-http      # Streamable HTTP
///   simplecommkitaiupdserver-fastmcpp --transport http --port 8102     # HTTP on port 8102

#include "udp_server_state.hpp"
#include "udp_server_tools.hpp"
...
int main(int argc, char* argv[])
{
    ...
    // -----------------------------------------------------------------------
    fastmcpp::FastMCP app("SimpleCommKitAiUdpServerFastmcpp MCP Server", "1.0.0",
    ...
    if (transport == "http")
    {
        std::cerr << "[SimpleCommKitAiUdpServerFastmcpp] Starting HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiUdpServerFastmcpp] Starting SSE server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiUdpServerFastmcpp] Starting Streamable HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        // Default: stdio
        std::cerr << "[SimpleCommKitAiUdpServerFastmcpp] Starting STDIO MCP server..." << std::endl;
    ...
    std::cerr << "[SimpleCommKitAiUdpServerFastmcpp] Server stopped." << std::endl;
    return 0;
}
