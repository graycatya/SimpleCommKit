---
name: simple-comm-kit-ai-hid
description: Use the SimpleCommKitAiHid MCP server to enumerate, open, and interact with HID (Human Interface Device) devices. This skill provides guidance on the recommended flow (enumerate -> open -> write/read) and handles platform-specific differences like device paths on different OSes. Use when the user wants to interact with HID hardware or debug USB HID connections.
---

# SimpleCommKitAiHid

SimpleCommKitAiHid is an AI-friendly HID toolkit powered by SimpleCommKitPyHid. This skill provides instructions for using the SimpleCommKitAiHid MCP server to interact with HID (Human Interface Device) devices directly from the host machine.
