/// SimpleCommKitAiTcpServerFastmcpp - C++ MCP server for TCP server
///
/// Exposes 6 MCP tools for TCP server start/stop, send/broadcast, status,
/// and buffered message retrieval.
/// Supports stdio, http, sse, and streamable-http transports.
///
/// Usage:
///   simplecommkitaitcpserver-fastmcpp                                  # stdio (default)
///   simplecommkitaitcpserver-fastmcpp --transport sse --port 8007      # SSE on port 8007
///   simplecommkitaitcpserver-fastmcpp --transport streamable-http      # Streamable HTTP
///   simplecommkitaitcpserver-fastmcpp --transport http --port 8007     # HTTP on port 8007

#include "tcp_server_state.hpp"
#include "tcp_server_tools.hpp"
...
using namespace SimpleCommKitAiTcpServerFastmcpp;
...
int main(int argc, char* argv[])
{
    ...
    // -----------------------------------------------------------------------
    fastmcpp::FastMCP app("SimpleCommKitAiTcpServerFastmcpp MCP Server", "1.0.0",
    ...
    if (transport == "http")
    {
        std::cerr << "[SimpleCommKitAiTcpServerFastmcpp] Starting HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiTcpServerFastmcpp] Starting SSE server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiTcpServerFastmcpp] Starting Streamable HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        // Default: stdio
        std::cerr << "[SimpleCommKitAiTcpServerFastmcpp] Starting STDIO MCP server..." << std::endl;
    ...
    std::cerr << "[SimpleCommKitAiTcpServerFastmcpp] Server stopped." << std::endl;
    return 0;
}
