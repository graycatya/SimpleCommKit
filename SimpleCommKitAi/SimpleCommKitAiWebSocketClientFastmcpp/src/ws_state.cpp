...
// ===========================================================================
// WsClientState singleton
// ===========================================================================

WsClientState& WsClientState::instance()
{
    static WsClientState s;
    return s;
}

void WsClientState::ensure_client()
{
    ...
    client_ = std::make_unique<SimpleCommKit::SimpleCommKitWebSocketClient>();
    install_callbacks();
}

bool WsClientState::has_client()
{
    std::lock_guard<std::mutex> lock(client_mutex_);
    return client_ != nullptr;
}

SimpleCommKit::SimpleCommKitWebSocketClient* WsClientState::get_client()
{
    std::lock_guard<std::mutex> lock(client_mutex_);
    return client_.get();
}

void WsClientState::destroy_client()
{
    std::lock_guard<std::mutex> lock(client_mutex_);
    client_.reset();
...
void WsClientState::install_callbacks()
{
    if (callbacks_installed_ || !client_)
        return;

    client_->setCallback_OnOpen(
        []()
        {
            std::cerr << "[SimpleCommKitAiWebSocketClientFastmcpp] WebSocket connection opened." << std::endl;
        });

    client_->setCallback_OnClose(
        []()
        {
            std::cerr << "[SimpleCommKitAiWebSocketClientFastmcpp] WebSocket connection closed." << std::endl;
        });

    client_->setCallback_OnMessage(
        [this](const std::vector<uint8_t>& data)
        {
            WsMessageEntry entry;
    ...
    client_->setCallback_OnError(
        [](SimpleCommKit::ErrorCode code)
        {
            ...
            std::cerr << "[SimpleCommKitAiWebSocketClientFastmcpp] Error: "
                      << code << " - " << desc << std::endl;
        });
...
std::vector<WsMessageEntry> WsClientState::drain_messages()
{
    std::lock_guard<std::mutex> lock(message_mutex_);
...
void WsClientState::clear_messages()
{
    std::lock_guard<std::mutex> lock(message_mutex_);
    message_buffer_.clear();
}
