/// SimpleCommKitAiMqttClientFastmcpp - C++ MCP server for MQTT
///
/// Exposes 10 MCP tools for MQTT broker connection, publish, subscribe,
/// message retrieval, and configuration.
/// Supports stdio, http, sse, and streamable-http transports.
///
/// Usage:
///   simplecommkitaimqttclient-fastmcpp                                     # stdio (default)
///   simplecommkitaimqttclient-fastmcpp --transport sse --port 8004         # SSE on port 8004
///   simplecommkitaimqttclient-fastmcpp --transport streamable-http         # Streamable HTTP
///   simplecommkitaimqttclient-fastmcpp --transport http --port 8004        # HTTP on port 8004

#include "mqtt_state.hpp"
#include "mqtt_tools.hpp"
...
int main(int argc, char* argv[])
{
    ...
    // -----------------------------------------------------------------------
    std::cerr << "[SimpleCommKitAiMqttClientFastmcpp] Initializing MQTT client..."
              << std::endl;
    if (!MqttState::instance().init())
    {
        std::cerr
            << "[SimpleCommKitAiMqttClientFastmcpp] WARNING: MQTT client init failed. "
    ...
    // -----------------------------------------------------------------------
    fastmcpp::FastMCP app("SimpleCommKitAiMqttClientFastmcpp MCP Server",
                           "1.0.0", std::nullopt, std::nullopt, std::nullopt,
    ...
    if (transport == "http")
    {
        std::cerr << "[SimpleCommKitAiMqttClientFastmcpp] Starting HTTP server on "
                  << host << ":" << port << std::endl;
        ...
        std::cerr << "HTTP MCP server running at http://" << host << ":"
                  << port << std::endl;
        ...
        MqttState::instance().cleanup();
        server.stop();
    }
    else if (transport == "sse")
    {
        std::cerr << "[SimpleCommKitAiMqttClientFastmcpp] Starting SSE server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr
            << "[SimpleCommKitAiMqttClientFastmcpp] Starting Streamable HTTP server on "
            << host << ":" << port << std::endl;

        fastmcpp::server::StreamableHttpServerWrapper server(handler, host,
                                                              port, "/mcp");
    ...
    else
    {
        // Default: stdio
        std::cerr
            << "[SimpleCommKitAiMqttClientFastmcpp] Starting STDIO MCP server..."
            << std::endl;
    ...
    std::cerr << "[SimpleCommKitAiMqttClientFastmcpp] Server stopped."
              << std::endl;
    return 0;
}
