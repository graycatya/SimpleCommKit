/// SimpleCommKitAiTcpClientFastmcpp - C++ MCP server for TCP client
///
/// Exposes 6 MCP tools for TCP client connect/disconnect, send, status,
/// buffered message retrieval, and reconnection configuration.
/// Supports stdio, http, sse, and streamable-http transports.
///
/// Usage:
///   simplecommkitaitcpclient-fastmcpp                                  # stdio (default)
///   simplecommkitaitcpclient-fastmcpp --transport sse --port 8006      # SSE on port 8006
///   simplecommkitaitcpclient-fastmcpp --transport streamable-http      # Streamable HTTP
///   simplecommkitaitcpclient-fastmcpp --transport http --port 8006     # HTTP on port 8006
...
using namespace SimpleCommKitAiTcpClientFastmcpp;
...
int main(int argc, char* argv[])
{
    ...
    // -----------------------------------------------------------------------
    fastmcpp::FastMCP app("SimpleCommKitAiTcpClientFastmcpp MCP Server", "1.0.0",
    ...
    if (transport == "http")
    {
        std::cerr << "[SimpleCommKitAiTcpClientFastmcpp] Starting HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiTcpClientFastmcpp] Starting SSE server on "
                  << host << ":" << port << std::endl;
    ...
    {
        std::cerr << "[SimpleCommKitAiTcpClientFastmcpp] Starting Streamable HTTP server on "
                  << host << ":" << port << std::endl;
    ...
    {
        // Default: stdio
        std::cerr << "[SimpleCommKitAiTcpClientFastmcpp] Starting STDIO MCP server..." << std::endl;
    ...
    std::cerr << "[SimpleCommKitAiTcpClientFastmcpp] Server stopped." << std::endl;
    return 0;
}
