...
### Q: Android 构建时提示 "fatal error: 'hv/TcpClient.h' file not found" 怎么办？

检查 `CMakeLists.txt` 中 line 282 的 libhv 保留条件是否为 `OR` 而非 `AND`。如果 MQTT 模块未启用（默认），原 `AND` 逻辑会导致 libhv 被错误排除。

### Q: Android 构建时提示 "unable to find library -lnativehelper" 或 "undefined symbol: JNI_GetCreatedJavaVMs" 怎么办？

NDK r23+ 移除了 `libnativehelper` 库。本项目已在 simpleble Android 后端中内置了 JNI 符号桩（`JniStubs.cpp`），无需额外操作。

### Q: Android 构建时提示 "unable to find library -lpthread" 怎么办？

Android 的 pthread 实现内置在 bionic libc 中，不存在独立的 `libpthread.so`。本项目已自动排除 Android 平台的 `-lpthread` 链接。
